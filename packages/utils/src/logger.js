...
export class Logger {
    ...
    static instance(filename, active = true) {
        ...
        if (count === 1) {
            ...
            if (active)
                console.log(colors.cyanBright(`¶ LOGGER ${filename} [${count}] UPDATED`));
        }
        return this.instances.get(filename);
    }
    constructor(filename, active = true) {
    ...
    debug(...args) {
        if (!this.approved(LogLevel.DEBUG))
            return;
        console.log(`🔍 ${this.formatMessage()}`, ...args);
    }
    success(...args) {
        if (!this.approved(LogLevel.SUCCESS))
            return;
        console.log(`✅ ${this.formatMessage()}`, ...args);
    }
    info(...args) {
        if (!this.approved(LogLevel.INFO))
            return;
     ...
     */
    warn(...args) {
        if (!this.approved(LogLevel.WARN))
            return;
        // Check if the first argument is an ErrorCode
        let errorCode;
        if (args.length > 0 && typeof args[0] === 'string' && args[0].startsWith('CD')) {
