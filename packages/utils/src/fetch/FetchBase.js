...
export class FetchBase {
    constructor(config) {
        this.logger = Logger.instance('FetchBase', false);
        this.config = config;
    }
    configure(config) {
    ...
    async patch(url, body, options) {
        return this.fetch(url, { ...options, method: 'PATCH', body: JSON.stringify(body) });
    }
    async put(url, body, options) {
        return this.fetch(url, { ...options, method: 'PUT', body: JSON.stringify(body) });
    }
    async delete(url, body, options) {
        return this.fetch(url, { ...options, method: 'DELETE', body: body ? JSON.stringify(body) : undefined });
    }
